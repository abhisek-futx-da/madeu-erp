...
export const CompanyBanner: React.FC<Props> = ({ legalName, gstin, fyLabel, online }) => (
  <header className="bg-[#e9eef4] border-b border-[#b0c4de] select-none text-[12px]">
    <div className="bg-gradient-to-r from-[#1e3a8a] via-[#1e40af] to-[#2563eb] text-white px-3 py-1.5 flex flex-wrap items-center gap-2 justify-between shadow-xs">
      <div className="min-w-0 flex-1 flex flex-wrap items-center gap-x-3 gap-y-1 font-semibold text-xs tracking-wide">
        <span className="bg-amber-400 text-slate-900 px-1.5 py-0.5 rounded text-[11px] font-bold uppercase shadow-xs">
          Link ERP
        </span>
        <span className="text-white drop-shadow-xs font-bold text-sm truncate max-w-full">{legalName}</span>
      ...
      <div role="status" className="shrink-0 flex items-center gap-1.5 bg-blue-900/80 px-2.5 py-1 rounded border border-blue-400/30 text-xs">
